...
type BehaviorMetric = { timeframe: string; price_change_pct: number; price_change_atr: number; spot_net_ratio: number; futures_net_ratio: number; oi_change_pct: number; spot_positive_share: number; spot_negative_share: number; futures_positive_share: number; futures_negative_share: number };
type FlowBehavior = { code: string; headline: string; detail: string; evidence_grade: "weak" | "medium" | "strong"; evidence: string[]; risks: string[]; missing_confirmations: string[]; metrics: Record<string, BehaviorMetric>; score_weight: 0; quality: Quality; note: string };
type FlowExhaustionMetrics = { futures_net_ratio_1h: number; futures_net_ratio_prev_1h?: number | null; futures_net_ratio_delta?: number | null; futures_net_ratio_slope_3h?: number | null; spot_net_ratio_1h: number; spot_net_ratio_prev_1h?: number | null; spot_net_ratio_delta?: number | null; spot_net_ratio_slope_3h?: number | null; oi_change_1h: number; oi_change_4h: number; price_change_atr_1h: number; funding_crowding: string; funding_percentile_30d?: number | null };
type FlowExhaustionWatch = { code: string; headline: string; detail: string; evidence_grade: "weak" | "medium" | "strong"; evidence: string[]; risks: string[]; missing_confirmations: string[]; metrics: FlowExhaustionMetrics; score_weight: 0; quality: Quality; note: string };
...
type Snapshot = {
  ...
  flow_behavior?: FlowBehavior; flow_exhaustion_watch?: FlowExhaustionWatch;
...
const BEHAVIOR_CN: Record<string, string> = { data_invalid: "数据不足", no_signal: "暂无有效脉冲", one_off_spike: "单次脉冲", short_covering: "空头回补", leveraged_rebound_unconfirmed: "杠杆反弹未确认", spot_confirmed_rebound: "现货确认反弹", buy_absorption_risk: "买盘吸收/诱多风险", long_closing: "多头止损/平仓", leveraged_selloff_unconfirmed: "杠杆下跌未确认", spot_confirmed_selloff: "现货确认卖压", sell_absorption_risk: "卖盘吸收/诱空风险", reversal_watch: "反转观察", reversal_confirmed: "反转已由状态机确认", mixed: "信号混合" };
const EXHAUSTION_CN: Record<string, string> = { data_invalid: "数据不足", no_signal: "暂无有效脉冲", bearish_continuation: "空头延续", sell_pressure_exhaustion_watch: "卖压衰竭观察", short_covering_bounce: "空头回补反弹", spot_confirmed_rebound: "现货确认反弹", bull_trap_risk: "诱多/吸收风险", sell_absorption_risk: "诱空/吸收风险", bullish_continuation: "多头延续", buy_pressure_exhaustion_watch: "买压衰竭观察", long_closing_selloff: "多头止损/平仓", spot_confirmed_selloff: "现货确认卖压", mixed: "信号混合" };
const CROWDING_CN: Record<string, string> = { unknown: "未知", neutral: "中性", long_crowded: "多头拥挤", short_crowded: "空头拥挤" };
...
function explain(data: Snapshot) {
  ...
  return {
    ...
    supports, conflicts, missing: missing.length ? missing : confirmed ? [] : ["等待连续闭合周期确认"],
  };
}
      ...
      <div className="mx-auto max-w-[1500px] space-y-5 p-3 md:p-5">
        ...
        {!data || !summary ? <div className="py-24 text-center text-slate-500">正在读取首个闭合周期快照…</div> : <>
          <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4 md:p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3"><Evidence title="支持当前方向" items={summary.supports} toneClass="text-emerald-400" empty="暂无强支持证据" /><Evidence title="冲突或保留意见" items={summary.conflicts} toneClass="text-amber-400" empty="暂无明显冲突" /><Evidence title={data.state.includes("confirmed") ? "确认状态" : "距离确认还缺什么"} items={summary.missing} toneClass="text-blue-400" empty="确认条件已满足" /></div>
          </section>
          ...
          <FlowExhaustionPanel watch={data.flow_exhaustion_watch} fallback={data.flow_behavior} />
...
function Stage({ state, count, target }: { state: string; count: number; target: number }) { const index = state.includes("confirmed") ? 2 : state.includes("candidate") || state.includes("reversal_watch") ? 1 : 0; const confirmationText = count >= target ? `已连续确认${count}个闭合周期（确认门槛${target}个）` : `连续核心确认：${count}/${target}`; return <div className="mt-4"><div className="flex items-center gap-2 text-[10px] text-slate-500">{["观察", "候选", "确认"].map((label, idx) => <div key={label} className="flex flex-1 items-center gap-2"><span className={`flex h-5 w-5 items-center justify-center rounded-full ${idx <= index ? "bg-blue-500 text-white" : "bg-slate-800"}`}>{idx + 1}</span><span>{label}</span>{idx < 2 && <span className="h-px flex-1 bg-slate-700" />}</div>)}</div><div className="mt-2 text-[10px] text-slate-600">{confirmationText}</div></div>; }
...
function FlowExhaustionPanel({ watch, fallback }: { watch?: FlowExhaustionWatch; fallback?: FlowBehavior }) {
  if (!watch || !watch.quality.valid) return <BehaviorPanel behavior={fallback} />;
  const grade = { weak: "证据较弱", medium: "证据中等", strong: "证据较强" }[watch.evidence_grade];
  const metric = watch.metrics;
  const ratio = (value?: number | null) => value == null ? "样本积累中" : `${value >= 0 ? "+" : ""}${(value * 100).toFixed(2)}%`;
  return <section className="rounded-xl border border-blue-900/60 bg-gradient-to-br from-slate-900/70 to-blue-950/20 p-4 md:p-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <SectionTitle title="卖压/买压是否正在衰竭" note="联动闭合小时价格、期现主动流、去价格化OI与Funding；固定零权重" />
      <div className="flex gap-2 text-[10px]"><span className="rounded bg-slate-950 px-2 py-1 text-blue-300">{EXHAUSTION_CN[watch.code] || watch.code}</span><span className="rounded bg-slate-950 px-2 py-1 text-slate-400">{grade} · 不是胜率</span></div>
    </div>
    <h3 className="mt-4 text-lg font-semibold text-slate-100">{watch.headline}</h3>
    <p className="mt-1 text-sm leading-6 text-slate-400">{watch.detail}</p>
    <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4 xl:grid-cols-8">
      <SmallMetric label="1h价格反应" value={`${metric.price_change_atr_1h >= 0 ? "+" : ""}${metric.price_change_atr_1h.toFixed(2)} ATR`} valueClass={tone(metric.price_change_atr_1h)} />
      <SmallMetric label="1h合约净流" value={ratio(metric.futures_net_ratio_1h)} valueClass={tone(metric.futures_net_ratio_1h)} />
      <SmallMetric label="较上一闭合1h" value={ratio(metric.futures_net_ratio_delta)} valueClass={tone(metric.futures_net_ratio_delta || 0)} />
      <SmallMetric label="合约3h斜率" value={ratio(metric.futures_net_ratio_slope_3h)} valueClass={tone(metric.futures_net_ratio_slope_3h || 0)} />
      <SmallMetric label="1h现货净流" value={ratio(metric.spot_net_ratio_1h)} valueClass={tone(metric.spot_net_ratio_1h)} />
      <SmallMetric label="现货较上一1h" value={ratio(metric.spot_net_ratio_delta)} valueClass={tone(metric.spot_net_ratio_delta || 0)} />
      <SmallMetric label="去价格化OI 1h/4h" value={`${ratio(metric.oi_change_1h)} / ${ratio(metric.oi_change_4h)}`} valueClass={tone(metric.oi_change_1h)} />
      <SmallMetric label="Funding拥挤" value={CROWDING_CN[metric.funding_crowding] || metric.funding_crowding} />
    </div>
    <div className="mt-4 grid gap-3 md:grid-cols-3"><Evidence title="为什么这样判断" items={watch.evidence} toneClass="text-emerald-400" empty="暂无有效证据" /><Evidence title="当前风险" items={watch.risks} toneClass="text-amber-400" empty="暂无额外风险证据" /><Evidence title="还缺什么确认" items={watch.missing_confirmations} toneClass="text-blue-400" empty="当前分类无需额外解释条件" /></div>
    <p className="mt-3 text-[10px] leading-5 text-slate-500">比较最近三个闭合1h窗口；下方NetFlow是滚动窗口，数值可能略有差异。{watch.note}</p>
  </section>;
}
function BehaviorPanel({ behavior }: { behavior?: FlowBehavior }) { if (!behavior) return <section className="rounded-xl border border-slate-800 bg-slate-900/55 p-4"><SectionTitle title="这波合约资金代表什么" note="等待新版解释快照…" /></section>; const grade = { weak: "证据较弱", medium: "证据中等", strong: "证据较强" }[behavior.evidence_grade]; const metric = behavior.metrics?.["1h"]; return <section className="rounded-xl border border-blue-900/60 bg-gradient-to-br from-slate-900/70 to-blue-950/20 p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><SectionTitle title="这波合约资金代表什么" note="联合价格反应、现货主动流、合约主动流与去价格化OI；固定零权重" /><div className="flex gap-2 text-[10px]"><span className="rounded bg-slate-950 px-2 py-1 text-blue-300">{BEHAVIOR_CN[behavior.code] || behavior.code}</span><span className="rounded bg-slate-950 px-2 py-1 text-slate-400">{grade} · 不是胜率</span></div></div><h3 className="mt-4 text-lg font-semibold text-slate-100">{behavior.headline}</h3><p className="mt-1 text-sm leading-6 text-slate-400">{behavior.detail}</p>{metric && <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"><SmallMetric label="1h价格反应" value={`${metric.price_change_atr >= 0 ? "+" : ""}${metric.price_change_atr.toFixed(2)} ATR`} valueClass={tone(metric.price_change_atr)} /><SmallMetric label="1h合约净流比例" value={`${metric.futures_net_ratio >= 0 ? "+" : ""}${(metric.futures_net_ratio * 100).toFixed(2)}%`} valueClass={tone(metric.futures_net_ratio)} /><SmallMetric label="1h现货净流比例" value={`${metric.spot_net_ratio >= 0 ? "+" : ""}${(metric.spot_net_ratio * 100).toFixed(2)}%`} valueClass={tone(metric.spot_net_ratio)} /><SmallMetric label="1h去价格化OI" value={`${metric.oi_change_pct >= 0 ? "+" : ""}${(metric.oi_change_pct * 100).toFixed(2)}%`} valueClass={tone(metric.oi_change_pct)} /></div>}<div className="mt-4 grid gap-3 md:grid-cols-3"><Evidence title="为什么这样判断" items={behavior.evidence} toneClass="text-emerald-400" empty="暂无有效证据" /><Evidence title="当前风险" items={behavior.risks} toneClass="text-amber-400" empty="暂无额外风险证据" /><Evidence title="还缺什么确认" items={behavior.missing_confirmations} toneClass="text-blue-400" empty="无需额外确认" /></div><p className="mt-3 text-[10px] leading-5 text-slate-500">解释器使用最近闭合1h/4h数据；下方NetFlow是滚动窗口，数值可能略有差异。{behavior.note}</p></section>; }
function FlowPanel({ title, flow }: { title: string; flow?: ActiveFlow }) { return <div className="rounded-xl border border-slate-800 bg-slate-900/55 p-4"><SectionTitle title={title} note={flow?.semantics || "数据未就绪"} /><div className="mt-1 text-[10px] text-slate-600">{qualityText(flow?.quality)}</div>{flow?.market === "futures" && <div className="mt-2 rounded bg-amber-950/20 px-2 py-1.5 text-[10px] leading-4 text-amber-300/80">主动净买不等于真实看多；OI增加也不能单独证明多头获胜，必须结合价格反应和现货确认。</div>}<div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-5">{flow?.windows.map((row) => <div key={row.window} className="rounded-lg bg-slate-950/70 p-2 text-center"><div className="text-[10px] text-slate-500">{row.window}</div><div className={`mt-1 text-xs font-semibold ${tone(row.net_usd)}`}>{row.net_usd >= 0 ? "净买 " : "净卖 "}{usd(Math.abs(row.net_usd))}</div><div className="mt-1 text-[9px] text-slate-600">净额/总量 {(row.net_ratio * 100).toFixed(1)}%</div><div className="mt-1 text-[8px] text-slate-700">买 {usd(row.buy_usd)} · 卖 {usd(row.sell_usd)}</div><div className="text-[8px] text-slate-600">{row.historical_percentile == null ? "历史样本积累中" : `历史P${row.historical_percentile.toFixed(1)}`}</div></div>)}</div><div className="mt-3 text-[10px] text-slate-500">与本地重建CVD一致：{flow?.cvd_consistent == null ? "待判断" : flow.cvd_consistent ? "是" : "否"}（只校验，不重复计权）</div></div>; }
