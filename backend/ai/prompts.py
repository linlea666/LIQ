...
- **数据覆盖规则（重要）**：用户提示中会给出「宏观数据覆盖说明」。若已提供恐惧贪婪指数、BTC 市占、DXY、纳指、标普、黄金中任意一项，**不得**写「宏观数据完全缺失/宏观真空」；仅当上述传统外盘指标（DXY/纳指/标普/黄金）均未出现在输入中时，可写「传统外盘股指与黄金指标未接入或未解析」，并与已提供的加密情绪指标区分表述。
