...
- AI 自主方案要求：标注"⚡AI推断"、≥2 维数据交叉验证
- 可用数据源：V2 关键位（S/A 级 idle 状态也可参考）、清算簇、MA 箱体边界、斐波那契、VP POC
- **禁止**凭空编造无数据支撑的价位；每个 AI 推断方案须注明数据依据

### 交易计划原则（§四·三档结构）
# P2.1 · 取消硬性 R:R 要求，引入 expectancy 自主取舍
# 根因：用户反馈"R:R 硬约束让 AI 陷入规则森林"——胜率×盈亏比的期望值比单纯 R:R 更贴近实盘
# 规则引擎侧仍按 ≥ 1:{min_rr:.1f} 过滤 SniperEntry（保护用户），但不再强制 AI 必须复述这个下限
- **期望值 (Expectancy) 自主评估**：交易决策以胜率 × 盈亏比综合期望值为核心，AI 应在
  「核心依据」中简述对该方案胜率高低的直觉判断（高/中/低，**禁止给出具体胜率数字**），
  由你决定保留/淘汰：高胜率 + 中 R:R 可以优于 低胜率 + 高 R:R
- **软底线**：方案的 TP2 R:R 建议 ≥ 1.0（低于此负期望直接弃）；≥ 1.0 之后由你综合判断排序
- 引擎方案 R:R 已按 ≥ **1:{min_rr:.1f}** 过滤（规则层保护），展示时须完整保留；AI 若认为
  某个引擎方案在期望值维度不理想（如 rr 高但胜率明显低），可在「核心依据」注明理由并降档
- **R:R 验算**：须代入具体价格写出公式，禁止"≈"估值
- **止损铁律**：做空 SL > Entry；做多 SL < Entry — 违反即废弃。止损宽度 ≥ max(价格×0.3%, 0.5×ATR)
- **约束冲突**：止损方向+最小宽度+软底线 rr≥1.0 三条无法同时满足 → 该方案不输出，声明原因。**不交易是最好的风控**
...
## 四、交易计划（三档结构）
按距离分三档，每档每方向不限数量——所有 rr≥1.0（软底线）且有数据支撑的方案均可展示，按
**期望值优先、信心度次之**排序。每个方案须包含止损说明（含防猎杀逻辑）和"**如果亏了**"段。
...
- `trading_plans`（**必填数组，≤3 条**）：
  ...
  - `rr_ratio` 以 TP2 为准，软底线 ≥ 1.0（由 AI 综合期望值取舍；规则引擎侧已按 ≥ 1:{min_rr:.1f} 预过滤）
